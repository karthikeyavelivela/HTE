...
export default function ProjectsManagement() {
    ...
    const [form, setForm] = useState<any>({
        title: "", category: "Student", status: "Ongoing", description: "",
        team: "", techStack: "",
    });
    ...
    const openAdd = () => {
        setForm({ title: "", category: "Student", status: "Ongoing", description: "", team: "", techStack: "" });
        setIsAdding(true); setEditing(null);
    };

    const openEdit = (p: Project) => {
        setForm({ ...p, team: p.team.join(", "), techStack: p.techStack.join(", ") });
    ...
    const save = () => {
        ...
        if (editing) {
            setItems((prev) => prev.map((p) => (p.id === editing.id ? { ...p, ...form, team, techStack: tech } : p)));
            showToast("Project updated!");
        } else {
            setItems((prev) => [...prev, { ...form, id: `PROJ-${Date.now()}`, team, techStack: tech, timeline: [], links: [] }]);
    ...
    const confirmDelete = () => {
        if (deleteId) { setItems((prev) => prev.filter((p) => p.id !== deleteId)); showToast("Project deleted!"); setDeleteId(null); }
    };
    ...
    return (
        <div style={{ color: "#000" }}>
            ...
            <div style={headerRow}>
                <h2 style={h2Style}>Projects Management</h2>
                <button onClick={openAdd} style={btnP}><Plus size={16} /> Add Project</button>
            </div>

            <div style={{ display: "flex", gap: "0.75rem", marginBottom: "1rem", flexWrap: "wrap" }}>
                <div style={{ position: "relative", flex: 1, minWidth: 200 }}>
                    <Search size={16} style={{ position: "absolute", left: 12, top: 12, color: "#888" }} />
                    <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search projects..." style={{ ...iS, paddingLeft: 36 }} />
                </div>
            ...
            <div style={{ overflowX: "auto" }}>
                <table style={tS}>
                    <thead><tr style={{ background: "#000", color: "#fff" }}>
                        {["Title", "Category", "Status", "Team", "Actions"].map((h) => (
                    ...
                    <tbody>
                        {filtered.map((p) => (
                            <tr key={p.id} style={{ borderBottom: "2px solid #000" }}>
                                <td style={{ ...tdS, fontWeight: 700 }}>{p.title}</td>
                                <td style={tdS}>{p.category}</td>
                                <td style={tdS}>
                                    <span style={{ display: "inline-block", padding: "0.15rem 0.6rem", background: p.status === "Ongoing" ? "#00F0FF" : "#BFFF00", border: "2px solid #000", fontFamily: "monospace", fontSize: "0.7rem", fontWeight: 700 }}>{p.status}</span>
                                </td>
                                <td style={{ ...tdS, fontFamily: "monospace", fontSize: "0.75rem" }}>{p.team.join(", ")}</td>
                                <td style={tdS}>
                                    <div style={{ display: "flex", gap: "0.35rem" }}>
                                        <button onClick={() => openEdit(p)} style={iBtn}><Edit size={15} /></button>
                                        <button onClick={() => setDeleteId(p.id)} style={{ ...iBtn, borderColor: "#dc2626" }}><Trash2 size={15} color="#dc2626" /></button>
                                    </div>
                                </td>
                            </tr>
                        ))}
            ...
            {(isAdding || editing) && (
                <Mdl title={editing ? "Edit Project" : "Add Project"} onClose={() => { setIsAdding(false); setEditing(null); }}>
                    ...
                    <div style={{ marginBottom: "0.75rem" }}>
                        ...
                        <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={3} style={{ ...iS, resize: "vertical" }} />
                    </div>
                    <div style={{ display: "flex", gap: "0.75rem", marginTop: "1rem" }}>
                        <button onClick={save} style={btnP}>Save</button>
                        <button onClick={() => { setIsAdding(false); setEditing(null); }} style={btnS}>Cancel</button>
                    </div>
                </Mdl>
            )}
            ...
            {deleteId && (
                <Mdl title="Confirm Delete" onClose={() => setDeleteId(null)}>
                    ...
                    <div style={{ display: "flex", gap: "0.75rem" }}>
                        ...
                        <button onClick={() => setDeleteId(null)} style={btnS}>Cancel</button>
                    </div>
                </Mdl>
            )}
        </div>
